...
Vector *vrand(size_t nrows, float min, float max) {
    ...
    // int success = posix_memalign((void**)&values, ALIGNMENT, nrows*sizeof(float));
    // printf("Success? %i\n", success);
...
Matrix *mrand(size_t nrows, size_t ncols, float min, float max) {
    ...
    //int success = posix_memalign((void**)&values, ALIGNMENT, ncols*nrows*sizeof(float));
    // printf("Success? %i\n", success);
                             ...
                             Vector *b)
{
    ...
    cblas_sgemv(CblasRowMajor,          // layout matches our storage
    ...
    for (size_t i0 = 0; i0 < A->nrows; i0 += tile_m) {
        ...
        for (size_t j0 = 0; j0 < A->ncols; j0 += tile_n) {
            ...
            for (size_t i = i0; i < imax; ++i) {
                float acc = y->values[i];           // start with existing y[i]

                const float *row = &A->values[i * A->ncols + j0];
...
int main() {
    ...
    // Test 
    // mmultv(A, x, b); 
    mmultv_tile(A, x, b, tile_m, tile_n); 
